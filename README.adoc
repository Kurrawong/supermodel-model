...
|**Definition** | A Supermodel is an enterprise data model that provides a structure for the creation of models for specialised datasets that allow for their uniqueness to be preserved but also provide for their deep integration.
|**Created** | 2021-12-10
|**Modified** | 2025-11-14
|**Issued** | -
...
The figure above shows the parts of the https://www.wa.gov.au/organisation/department-of-mines-petroleum-and-exploration/geological-survey-of-western-australia[Geological Survey of Western Australia]'s Supermodel of their geological and resource sector regulation data. It shows models for specialised geological datasets such as _Boreholes_, _Samples_ and _Mining Authorisation_ as well as more generic _Background Models_ such as https://www.w3.org/TR/vocab-ssn/[_SOSA_] (for observations and measurements), http://www.opengis.net/doc/IS/geosparql/1.1[_GeoSPARQL_] (for spatial features) and so on. It also indicates some of the vocabularies of controlled terms used by both _Foreground_ and _Background Models_, such as _Minerals_ and _Commodities_.
...
While many useful models are not Semantic Web models, all models can be realised as such and the Semantic Web formulation of models and data in the <<OWL, Web Ontology Language>> is necessary for supermodels so that many existing models can be used and so things created for a particular supermodel themselves can be maximally reused.
...
One aspect of formal supermodel element descriptions are the roles that elements play within a supermodel scope. Particular models may be for the representation of the data within datasets to be integrated are called - _Foreground Model_ role - or they may be used by other models as reference - _Background Model_ role - or perhaps both (see the GSWA overview figure at the top).

The roles for supermodel elements are indicated in the formal modelling of it, as per the <<Model>> section below, and the roles themselves are detailed in the <<Roles>> section.
...
** assume one will be implemented unless all elements of the model can be modelled using <<Background Model, Background Models>> or other Dataset's Foreground Models
...
All general purpose models that have had their whole or parts reused by Foreground Models must be included as <<Background Model, Background Models>> within the supermodel.
...
[.text-center]
image::images/relationships.svg["Defined relationships between Supermodel element classes ",width=100%]
[.text-center]
_Defined relationships between Supermodel element classes_
...
| `sm` | `https://linked.data.gov.au/def/supermodel/` | Supermodel
...
An example of a real supermodel instance - that of https://linked.data.gov.au/org/qsi[Queensland Spatial Information]'s _Queensland Spatial Information Supermodel_ online at https://linked.data.gov.au/def/qsi-supermodel - abbreviated for clarity by hiding the vocabularies and some profile relations, is as follows:
...
////
NOTE (Les):
`sh:Shape` is a superclass in SHACL, with `sh:NodeShape` and `sh:PropertyShape`
as its primary concrete subclasses.

In most SHACL usage, those subclasses are used directly.
`sh:Shape` is technically valid but uncommon in examples.

Leaving as-is here as this may be an intentional abstraction in the Supermodel.
////
...
Use the predicate `prof:isProfileOf` to indicate the dependence of a <<Profile>> on a <<Model>>.
...
* a <<Class>> or <<Concept>> instance is part of an object with role <<role-dataset>>
* a <<Class>> is part of an object with role <<role-model>>
* a <<Shape>> is part of an object with role <<role-validator>>
...
Use the predicate `sm:targets` to indicate that a <<Shape>> constrains a <<Class>>.
...
* https://linked.data.gov.au/def/supermodel/validator
...
* <<role-dataset>>
** <<role-foreground-dataset>>
** <<role-reference-dataset>>
*** <<role-vocabulary>>
* <<role-model>>
** <<role-supermodel-definition>>
** <<role-foreground-model>>
...
`https://linked.data.gov.au/def/supermodel/roles/dataset`
...
`https://linked.data.gov.au/def/supermodel/roles/foreground-dataset`
...
`https://linked.data.gov.au/def/supermodel/roles/reference-dataset`

Acting as a <<Dataset>> that is used as an integration point by one or more <<Dataset, Datasets>>.

[[role-vocabulary]]
[discrete]
=== Vocabulary

`https://linked.data.gov.au/def/supermodel/roles/vocabulary`

Acting as a controlled collection of terms, usually with a hierarchy but sometimes just a list, used as <<Reference Dataset>> by <<Dataset, Datasets>>.
...
See the https://linked.data.gov.au/def/vocpub[VocPub profile] as a model of classification vocabularies that can be followed for supermodels.
====
...
`https://linked.data.gov.au/def/supermodel/roles/model`
...
`https://linked.data.gov.au/def/supermodel/roles/supermodel-definition`
...
`https://linked.data.gov.au/def/supermodel/roles/foreground-model`
...
`https://linked.data.gov.au/def/supermodel/roles/background-model`
...
`https://linked.data.gov.au/def/supermodel/roles/validation`

Acting as resource to validate the conformance of <<Dataset, Datasets>> to <<Profile, Profiles>>.
...
`https://linked.data.gov.au/def/supermodel/roles/data`
...
`https://linked.data.gov.au/def/supermodel/roles/foreground-data`

Acting as <<Data>> that is the main subject of the supermodel that may or may not also be used as <<role-reference-data>>.

[[role-reference-data]]
[discrete]
=== Reference Data

`https://linked.data.gov.au/def/supermodel/roles/reference-data`
...
[[DCTERMS]]
[DCTERMS]:: DCMI Usage Board, _DCMI Metadata Terms_, RDF Vocabulary (20 January 2020). https://www.dublincore.org/specifications/dublin-core/dcmi-terms/
...
[[RDF]]
[RDF]:: World Wide Web Consortium, _RDF 1.1 Concepts and Abstract Syntax_, W3C Recommendation (25 February 2014). http://www.w3.org/TR/rdf11-concepts/

[[RDFS]]
[RDFS]:: World Wide Web Consortium, _RDF Schema 1.1_, W3C Recommendation (25 February 2014). http://www.w3.org/TR/rdf11-schema/

[[SDO]]
[SDO]:: schema.org Consortium, _schema.org_, Ontology (14 November 2025). https://schema.org

[[SH]]
[SH]:: World Wide Web Consortium, _Shapes Constraint Language (SHACL)_, Ontology (20 July 2017). https://www.w3.org/TR/shacl/


[[SKOS]]
[SKOS]:: World Wide Web Consortium, _SKOS Simple Knowledge Organization System Reference_, W3C Working Group Note (19 August 2009). https://www.w3.org/TR/skos-reference/
...
_Foreground Models_ much profile - depend on - as many common _Background Models_ or other _Foreground Models_ as possible and _Background Models_ may profile other _Background Models_ too. The Supermodel is then a set of models with all dependencies shown and no _Foreground Models_ depending on nothing: this would indicate they are un-integrated. Fundamental _Background Models_ will likely not depend on anything, other than perhaps modelling system models, such as <<OWL, _Web Ontology Language_>> or <<RDFS, _Resource Description Framework Schema_>>.
////
...
Integrative Model:: Role for a model that is specialised and extended by multiple models with Foreground Model role

Foreground Model:: Role for individual datasets within a Supermodels' remit which specialise and extend models with the Integrative and Background Model roles

Background Model:: Role for a model that is specialised and extended by some models with Foreground Model role but not all
